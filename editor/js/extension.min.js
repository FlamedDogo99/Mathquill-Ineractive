...
let urlButton = document.getElementById('updateUrl');
...
urlButton.addEventListener("pointerdown", preventButton, false)
printButton.addEventListener("pointerdown", preventButton, false)
colorButton.addEventListener("pointerdown", preventButton, false)
...
function insertLatex(beginLatex, endLatex) {
...
var mathNode = document.querySelector("#math-field > span.mq-root-block");
...
mathFieldElement.addEventListener("touchstart", TouchEventMapper, true)
...
function setupMathField() {
	...
	if(encodedLatex) {
		var decodedLatex = decodeURIComponent(encodedLatex);
...
function catchSoftwareUndo(event) {
	...
	if (event.inputType == 'historyUndo') { // Some ipads have undo and re\
