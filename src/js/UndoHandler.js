...
export class UndoHandler {
  ...
  redo(currentValue) {
    ...
    if (stateIndex < historyLength) {
      //If we're at a stateIndex less than the length of the changes, it mean's we can redo states
  ...
  change(currentState) {
    ...
    if (inputChanged) {
      // We want to check this because we are using a mutation observer to catch changes made programatically, could be dispatched twice
      ...
      if (historyLength > stateIndex) {
        // If we're at a stateIndex less than the length of the changes, it mean's we've undone states.
  ...
  reverseChanges(stringNew, differences) {
    const reversedArray = differences.map(
      (item, idx) => differences[differences.length - 1 - idx],
    );
